...
import { updatedRequest, User } from '../Interface'
import crudModel from '../model/crudmodel'
import mongoose from 'mongoose'

dotenv.config()

const Middlewere = (req: updatedRequest, res: express.Response, next: express.NextFunction) => {
    ...
    if (Token && ENV_SECURT) {
        try {
            ...
            if(req.path !== "/login" && req.path !== "/register" && req.path !== "/forgotpassword" && req.path !== "/forgotpassword/:reseturl"){
                ...
                .then((response) => {
                        ...
                        //     response.password = ""
                        // }
                        req.user = response as unknown as User
                        next()
                    })
            } else {
