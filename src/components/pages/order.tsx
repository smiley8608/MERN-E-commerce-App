        ...
        <TableBody>
          {rows.map((row:any) => (
            <StyledTableRow key={row._id}>
              ...
              <StyledTableCell component="th" scope="row">{row.product.map((items:any)=>{
                
                
                return <p key={Math.random()}>{items.product.title}</p>
              })}</StyledTableCell>
               <StyledTableCell component="th" scope="row" align="center">{row.product.map((items:any)=>{
                
                
                return <p key={Math.random()}>{items.quantity}</p>
              })}</StyledTableCell>
              <StyledTableCell align="center">{row.amount}</StyledTableCell>
              <StyledTableCell align="right"><a href={`https://goerli.etherscan.io/tx/${row.paymentDetails.hash}`} target='_blank' rel="noreferrer">{row.paymentDetails.hash}</a></StyledTableCell>
              <StyledTableCell align="center">{row._id}</StyledTableCell>
              <StyledTableCell align="center">{row.deliverIn}</StyledTableCell>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
