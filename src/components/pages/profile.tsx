...
export const Profile = () => {
  ...
  const updatePasswordHandler = (e: FormEvent) => {
    ...
    if (
      ...
      newPassword.newPassword !== newPassword.confNewPassword
    ) {
      return message.error({
        content: "Complete all the fields properly.",
        style: { marginTop: 60 },
      });
    }
    axios
      ...
      <div className="tw-flex tw-items-start tw-gap-3 tw-flex-col md:tw-flex-row">
        <div className="tw-w-1/2">
          <div className="tw-w-full tw-flex tw-items-center tw-justify-between">
            ...
            <div
              className="tw-px-4 tw-py-2 tw-flex tw-items-center tw-rounded-full tw-cursor-pointer"
              style={{
          ...
          <form className="tw-py-3 tw-w-full" onSubmit={updateProfileHandler}>
            <div className="tw-w-full tw-mt-3">
              ...
              <input
                className="tw-p-1.5 tw-rounded-full tw-w-1/2 tw-px-3"
                readOnly={editmode ? false : true}
                value={editmode ? data?.username : user?.username}
            ...
            <div className="tw-flex tw-gap-x-3 tw-w-full">
              <div className="tw-w-full tw-mt-3">
                ...
                <input
                  className="tw-p-1.5 tw-rounded-full tw-w-full tw-px-3"
                  readOnly={editmode ? false : true}
                  value={editmode ? data?.fname : user?.firstname}
              ...
              <div className="tw-w-full tw-mt-3">
                ...
                <input
                  className="tw-p-1.5 tw-rounded-full tw-w-full tw-px-3"
                  readOnly={editmode ? false : true}
                  value={editmode ? data?.lname : user?.lastname}
            ...
            <div className="tw-w-1/2 tw-mt-3">
              ...
              <input
                className="tw-p-1.5 tw-rounded-full tw-w-full tw-px-3"
                readOnly={editmode ? false : true}
                value={editmode ? data?.email : user?.email}
            ...
            <div className="tw-w-1/2 tw-mt-3">
              ...
              <input
                className="tw-p-1.5 tw-rounded-full tw-w-full tw-px-3"
                readOnly={editmode ? false : true}
                value={editmode ? data?.phonenumber : user?.phonenumber}
                onChange={(e) =>
            ...
            {editmode && (
              <button className="tw-px-3 tw-py-1.5 tw-bg-teal-500 tw-rounded-md tw-shadow-lg tw-shadow-gray-400 tw-text-white tw-mt-3">
                Update
              </button>
            )}
          </form>
        ...
        <div className="tw-w-1/2">
          <div className="tw-w-full tw-flex tw-items-center tw-justify-between">
            ...
            <div
              className="tw-px-4 tw-py-2 tw-flex tw-items-center tw-rounded-full tw-cursor-pointer"
              style={{
                backgroundColor: passChangeMode
                  ? "rgb(20 184 166)"
                  : "rgb(251 191 36)",
              }}
              onClick={() => {
          ...
          <form className="tw-py-3 tw-w-full" onSubmit={updatePasswordHandler}>
            <div className="tw-w-full tw-mt-3">
              ...
              <TextField
                className="tw-bg-white tw-outline-none tw-rounded-full"
                disabled={!passChangeMode}
                name="password"
                id="password-old"
            ...
            {passChangeMode && (
              <button
                type="submit"
                className="tw-px-3 tw-py-1.5 tw-bg-teal-500 tw-rounded-md tw-shadow-lg tw-shadow-gray-400 tw-text-white tw-mt-3"
              >
                Update
              </button>
            )}
